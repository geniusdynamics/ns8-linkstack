...
# Reuse existing nodebuilder-linkstack container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-linkstack; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-linkstack -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/linkstackorg/linkstack:latest" " \
    "${container}"
# Commit the image
